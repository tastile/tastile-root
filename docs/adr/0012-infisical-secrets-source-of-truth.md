...
1. The user's self-hosted Infisical instance is the canonical source for all Tastile repository secrets and runtime configuration that must remain private. Use three Infisical projects, one each for `dev`, `staging`, and `prod` (development, staging, production), with project IDs recorded in each consuming child repository's `.infisical.json`; the shell repository has no runtime environment and stores no Infisical project configuration. This project boundary matches the available Free-tier access model and keeps environment credentials isolated without folder ACLs. Within a project, keep stable service paths (`/tastile/core`, `/tastile/web`, `/tastile/android`, `/tastile/desktop`, `/tastile/brands`, plus narrowly scoped integration paths). Its public HTTPS hostname is a generic host owned by the user and must not include `tastile`.
