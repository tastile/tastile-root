...
Infisical self-hosting does not itself unlock paid access-control features. This instance's UI currently gates folder-level access controls behind Pro. Keep all GitHub OIDC identities at No Access until a least-privilege grant is available; do not substitute broad project access.
...
Self-hosted HTTPS and interactive user login now work. Remaining work includes an Infisical plan that supports folder-level access controls or another approved least-privilege mechanism, GitHub Environment variables, values absent from target paths, secret-safe migration for GitHub/SOPS sources, Android/Cloudflare/platform integrations, and live consumer cutover checks. The current web SOPS age private keys are held only in GitHub Secrets, so migration must run in a controlled job after a scoped write grant is available. Never paste a secret or access token into a repository file or chat.
