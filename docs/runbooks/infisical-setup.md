...
One self-hosted Infisical project is the target for Tastile. Its public HTTPS hostname must be a generic hostname on a domain owned by the user and must not contain `tastile`, because the instance is not Tastile-specific. The domain is required in every integration, and clients must also set the project ID or project slug required by their Infisical CLI/action; missing selectors must fail closed instead of falling back to Infisical Cloud. The current project is named `tastile`, has ID `f2890cb7-599b-4bd6-b7b4-a47aeb9b324b` and slug `tastile-wn-hr`. Environment slugs are `dev`, `staging`, and `prod`, corresponding to development, staging, and production.
...
Create one GitHub OIDC machine identity for each actual GitHub deployment environment that reads Infisical. Bind the subject to the exact repository and GitHub environment name. Current environment names are core `staging` / `production`, web `preview` / `staging` / `production`, Android `android-release`, and desktop `production`. Map web `preview` and Android `android-release` to Infisical `staging` and `prod` respectively; map other names to the matching Infisical slug. Grant only that environment's service path and required integration child paths. Add the following non-secret GitHub Environment variables to each applicable repository:
...
- 2026-09-24 の値を表示しない inventory では、core は dev/prod 各8キー、web は dev 15/prod 23キー、Android は dev 4/prod 9キー、desktop は dev 6キー。全 service の staging と desktop prod は空で、brands の全 environment も空。Android の legacy `tastile-android` project から canonical path へコピーした dev 4/prod 9キーは、source と全値一致を確認済み。web prod の23キーは target inventory で確認したが、SOPS source との比較はまだ。
- restore helper は実データで web dev/prod 15/23、core dev/prod 各8、Android dev/prod 4/9、desktop/dev 6キーを復元し、各生成ファイルを削除した。さらに選択環境と同じキーだけを `.env.example` に生成するよう更新後、全4 service のdev環境を再復元し、`.env` 削除と空値schema生成を確認した。schemaはcore 8、web 15、Android 4、desktop 6件。secret 値は出力していない。この検証はローカル復元を確認するもので、child 設定・CI・production runtime の切替確認ではない。
- root の `.infisical.json` は self-hosted project ID/domain を指す。GitHub variables / workflow を read-only inventory したところ、core の production/staging と web の production/staging に identity ID はあるが、domain/project selector はなく web preview の identity ID もない。core/web/desktop の追加 action は現行 workflow から呼ばれていない。Android の repository variables は旧 project slug のままで、release workflow の Environment 名 `android-release` も既存 identity subject `production` と一致しない。Android の `.infisical.json` は今回 canonical project ID に更新したが、まだ push / CI 検証前。2026-09-24 に canonical project で core/web/android/desktop × development/staging/production の12 machine identity を作成し、GitHub OIDC audience `https://github.com/tastile` を設定した。全identityのproject roleは No Access。実際のworkflowで使用するidentityのsubject / environment mappingをそろえてから、最小権限を設定して検証する。
