...
- self-hosted server の UI は `v0.165.15 Free` と表示し、folder-level access control を有料機能として制限する。Infisical の self-hosted activation docs は paid features を使うには server の `LICENSE_KEY` 環境変数へ発行済み license key を設定するよう案内する。別契約の Cloud plan はこの server の状態を変更しない。今回の作業環境から server deployment の環境設定は確認・変更できていないため、12 OIDC identity は No Access のままにする。GitHub Environment variables と workflow の切替も未完了。
