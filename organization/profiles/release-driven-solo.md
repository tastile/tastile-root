...
Tastile workspace の current release-driven development で使用する Operating Model を定義します。
...
- root repository は cross-repo policy / shared operations / agent environment の canonical coordination layer として維持する。
- child repositories は独立 Git repository であり、root Git state だけから child state を推論しない。
- cross-repo work は repository ごとの current SHA / PR / evidence を追跡し、workspace-level orchestration が mutable ownership を競合させない。
